...
def requeue_adjustment(adjustment: RewardAdjustment) -> datetime:
    ...
    job = q.enqueue_at(  # requires rq worker --with-scheduler
        next_attempt_time,
        adjust_balance,
        account_holder_activation_id=adjustment.id,
...
def handle_request_exception(
    adjustment: RewardAdjustment, request_exception: httpx.RequestError
) -> Tuple[dict, Optional[RewardAdjustmentStatuses], datetime]:
    status = None
...
def handle_activate_account_holder_error(
    ...
    with SyncSessionMaker() as db_session:
        ...
        if isinstance(exc_value, httpx.RequestError):  # handle http failures specifically
